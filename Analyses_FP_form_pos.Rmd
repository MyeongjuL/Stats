...
```{r}
form_count <- count(df_form_pos, Form)
...
df_form_pos$Form <- str_replace_all(df_form_pos$Form, c("어"="/ʌ/", "아"="/a/", "음"="/ɨm/", "엄"="/ʌm/", "그"="/kɨ/", "응"="/ɨŋ/", "으"="/ɨ/", "etc"="etc"))
...
ggplot(form_count, aes(x = reorder(Form, -n), y = n, fill = ResponseCond)) +
  ...
  ggtitle("Filler Particle Form by Truthfulness") +
  scale_fill_manual(values = c("D"="#FF7C80", "T"="#8ED973"), labels = c("D" = "Deceptive", "T" = "Truthful"),  name = "Truthfulness") +
  theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14),
        axis.title.y = element_text(margin = margin(r = 15)), axis.title.x = element_text(margin = margin(t = 10)),
        plot.title = element_text(size = 16, margin = margin(b = 20)), legend.text = element_text(size = 12), legend.title = element_text(size = 12))
```
...
ggplot(pos_count, aes(x = Position, y = n, fill = ResponseCond)) +
  ...
  ggtitle("Filler Particle Position by Truthfulness") +
  scale_x_discrete(labels=c("INI"="Turn-Initial", "INT"="Turn-Internal")) +
  scale_fill_manual(values = c("D"="#FF7C80", "T"="#8ED973"), labels = c("D" = "Deceptive", "T" = "Truthful"),  name = "Truthfulness") +
  theme(axis.text = element_text(size = 12), axis.title = element_text(size = 14), axis.title.y = element_text(margin = margin(r = 15)), axis.title.x = element_text(margin = margin(t = 10)), plot.title = element_text(size = 16, margin = margin(b = 20)), legend.text = element_text(size = 12), legend.title = element_text(size = 12))
```
...
chisq_form$statistic # when df = 7, chi-square statistic should be >= 14.067 to be significant at the .05 level
chisq_form$p.value
