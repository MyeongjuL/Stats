...
plot(density(df_RL_main$RLMilSec), col="red", main="Density comparison", ylim=c(0, max_y), xlab("Response Latency (ms)"))
...
```{r}
table(df_RL_main$ResponseCond)
table(df_RL_folu$ResponseCond)
```
...
plot(density(df_RL_main$RLMilSec), col="red", main="Density comparison", ylim=c(0, max_y2), xlab("Response Latency (ms)"))
lines(density(df_RL_folu$RLMilSec), col="blue")
legend("topright", legend=c("Main", "Follow-up"), col=c("red","blue"), lty=1)
```
...
### How many responses per condition after removing outliers 2
```{r}
table(df_RL_main$ResponseCond)
table(df_RL_folu$ResponseCond)
```
...
plot(density(df_RL_main$RLMilSec), col="red", main="Density comparison", ylim=c(0, max_y3), xlab("Response Latency (ms)"))
lines(density(df_RL_folu$RLMilSec), col="blue")
legend("topright", legend=c("Main", "Follow-up"), col=c("red","blue"), lty=1)
```


## Response Latency (in each subset): means
```{r}
...
## Response Latency (together): means
```{r}
summarySE(df_RL_main_folu, measurevar = "RLMilSec", groupvars = c("ResponseCond"), na.rm = T)
```
...
ggplot(plot_RL, aes(x = ResponseCond, y = RLMilSec, fill = ResponseCond)) +
  ...
  ggtitle("Response Latency by Response Condition") +
...
ggplot(plot_RL_main, aes(x = ResponseCond, y = RLMilSec, fill = ResponseCond)) +
  ...
  ylab("Response Latency (ms)") +
  xlab("Response Condition") +
  ggtitle("Response Latency after Main Question by Response Condition") +
  scale_x_discrete(labels=c("T"="Truthful", "D"="Deceptive")) +
  geom_errorbar(aes(ymin=RLMilSec-se, ymax=RLMilSec+se), linewidth=.5, width=.2, position=position_dodge(0.6)) +
  scale_fill_manual(values = c("D"="#FF7C80", "T"="#8ED973")) +
  theme(legend.position = "none", axis.text = element_text(size = 12), axis.title = element_text(size = 14), 
...
ggplot(plot_RL_folu, aes(x = ResponseCond, y = RLMilSec, fill = ResponseCond)) +
  ...
  ylab("Response Latency (ms)") +
  xlab("Response Condition") +
  ggtitle("Response Latency after Follow-up Question by Response Condition") +
  scale_x_discrete(labels=c("T"="Truthful", "D"="Deceptive")) +
  geom_errorbar(aes(ymin=RLMilSec-se, ymax=RLMilSec+se), linewidth=.5, width=.2, position=position_dodge(0.6)) +
  scale_fill_manual(values = c("D"="#FF7C80", "T"="#8ED973")) +
  theme(legend.position = "none", axis.text = element_text(size = 12), axis.title = element_text(size = 14),
...
ggplot(plot_RL_main_folu, aes(x = QuestionType, y = RLMilSec, fill = ResponseCond)) +
  ...
  ylab("Response Latency (ms)") +
  xlab("Question Type") +
  ggtitle("Response Latency by Question Type and Response Condition") +
  ...
  scale_fill_manual(values = c("D" = "#FF7C80", "T" = "#8ED973"), labels = c("T" = "Truthful", "D" = "Deceptive"), name = "Response Condition") +
